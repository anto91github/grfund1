    ...
    <div class="container" >
		<div class="col-sm-6 col-xs-6">
		  <div class="page-title" style="padding-bottom:10px;">
			<h3><i>Preview of &nbsp; </i> {{ $project->title }} <small></small></h3>
		  </div>
		</div>
		<div class="col-sm-6 col-xs-6 ">
        ...
        <div class="row" style="padding-left: 30px; padding-right: 30px;">
            ...
            <div class="tab-content col-md-8" style="padding-left:0px; padding-right:0px;">
                <div class="tab-pane fade active in" id="tabHome" style="border:1px solid #D8D8D8; background-color: #FFFFFF; padding-top:10px; padding-bottom:10px; padding-left:20px; padding-right:20px; ">
                    ...
                    <div><p style="color:#000; font-weight:bold; font-size:18px;">{{ Input::get('is') }}</p></div>
                    <hr/>
                    <h4>Who we are:</h4>
                    <div><p style="color:#000;">{{ Input::get('w') }}</p></div>
                    <hr/>
                    <h4>What We Need:</h4>
                    <div><p style="color:#000;">{{ Input::get('n') }}</p></div>
                    <hr/>
                    <h4>The Impact:</h4>
                    <div><p style="color:#000;">{{ Input::get('im') }}</p></div>
                    <hr/>
                    <h4>Time Line: </h4>
                    <div><p style="color:#000;">{{ Input::get('t') }}</p></div>
                    <hr/>
                    <h4>Acknowledgements / rewards :</h4>
                    <div><p style="color:#000;">{{ Input::get('a') }}</p></div>
                    <hr/>
                    <h4>Additional Link :</h4>
                    <div class="row">
                        @if(Input::get('fb') != '')
                        <div class="col-md-2">Link to Facebook : </div><div class="col-md-10"><a href="{{Input::get('fb')}}" alt="link facebook" >{{Input::get('fb')}}</a></div>
                        @endif
                        @if(Input::get('tw') != '')
                        <div class="col-md-2">Link to Twitter : </div><div class="col-md-10"><a href="{{Input::get('tw')}}" alt="link twitter" >{{Input::get('tw')}}</a></div>
                        @endif
                        @if(Input::get('go') != '')
                        <div class="col-md-2">Link to Google+ : </div><div class="col-md-10"><a href="{{Input::get('go')}}" alt="link google" >{{Input::get('go')}}</a></div>
                        @endif
                        @if(Input::get('ins') != '')
                        <div class="col-md-2">Link to Instagram : </div><div class="col-md-10"><a href="{{Input::get('ins')}}" alt="link instagram" >{{Input::get('ins')}}</a></div>
                        @endif
                        @if(Input::get('you') != '')
                        <div class="col-md-2">Link to Youtube : </div><div class="col-md-10"><a href="{{Input::get('you')}}" alt="link youtube" >{{Input::get('you')}}</a></div>
                        @endif
                        @if(Input::get('link') != '')
                        <div class="col-md-2">Link to LinkedIn : </div><div class="col-md-10"><a href="{{Input::get('link')}}" alt="link linkedin" >{{Input::get('link')}}</a></div>
                        @endif
                    </div>
                    <hr/>
                    <h4>Images :</h4>
                    <div class="row" >                                               
                        <div class="col-md-4"><img id="photo1" src="uploads/reward/default_project.jpg" width="100%"></div>
                        <div class="col-md-4"><img id="photo2" src="uploads/reward/default_project.jpg" width="100%"></div>
                        <div class="col-md-4"><img id="photo3" src="uploads/reward/default_project.jpg" width="100%"></div>
                    ...
                    <div>
                        <h4>Video :</h4>
                        <div>
                            <p><strong><em><iframe frameborder="0" height="350" scrolling="no" src="{{ Input::get('url') }}" width="680"></iframe></em></strong></p>
                        </div>
                    </div>
                    @endif
                </div>
            ...
            <div class="col-md-4">
                <div style="border:1px solid #E8E8E8;  min-height:260px; overflow:hidden; margin-bottom: 10px; background-color: #FFFFFF;">
                        <div style="padding:10px;">
                            ...
                            <div style="text-align: center;"><h4 style="font-size: 16px;"><small>project finished at {{ $project->due_date }}</small></h4></div>
                             @if( $project->days_to_go->format("%r%a") > 0)
                            <div class="text-center"><a class="btn btn-primary">Fund this project</a></div>
                             @endif                           
                        </div>
                </div>
                ...
                <div class="col-md-12 container-reward" style="margin-bottom:10px;">
                    ...
                    @if($project->days_to_go->format("%r%a") > 0)
                        @if($c > 0)
                            <div style="width:100%; float:left; margin-top:5px;" align="center" ><a  class="btn btn-primary" style="width: 200px; border-radius:10px !important;">Fund Rp. {{ number_format($reward->reward_minimum,0,",",".") }} or more</a></div>
                        @else
...
<script type="text/javascript">
    ...
    $.post("protected/app/webservice/testing.php",{id:''},function(result){
     ...
     //document.getElementById("photo1").src = localStorage.getItem("photo1_prev");
     //document.getElementById("photo1").src = "uploads/users/1.jpg";

    jQuery(document).ready(function(){
        $("#photo1").attr("src", localStorage.getItem("photo1_prev"));
        $("#photo2").attr("src", localStorage.getItem("photo2_prev"));
        $("#photo3").attr("src", localStorage.getItem("photo3_prev"));
