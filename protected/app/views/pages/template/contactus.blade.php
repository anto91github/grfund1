    ...
    <div class=" container">
		<div class="col-sm-6 col-xs-6">
		  <div class="page-title">
			<h3> {{ Lang::get('core.contact_us') }} <small></small></h3>
		  </div>
		</div>
		<div class="col-sm-6 col-xs-6 ">
		  <ul class="breadcrumb pull-right">
			<li><a href="{{ URL::to('') }}">{{ Lang::get('core.home') }}</a></li>
			<li class="active">{{ Lang::get('core.contact_us') }} </li>
		  </ul>		
		</div>
...
<div class="wrapper-container container">
	<div class="row">
		...
		<div class="col-md-6">
		 <h3> {{ Lang::get('core.contact_send') }}</h3>
{{ Form::open(array('url'=>'home/contactform', 'class'=>'form-vertical','parsley-validate'=>'','novalidate'=>' ')) }}
	...
	@endif	
		...
		<label for="ipt"> {{ Lang::get('core.contact_name') }}</label>
		  {{ Form::text('name', null,array('class'=>'form-control', 'placeholder'=>'', 'required'=>'Your Name'  )) }} 
		
	  </div> 			
						 					
	  <div class="form-group  ">
		<label for="ipt"> {{ Lang::get('core.contact_email') }}</label>
		
		  {{ Form::text('sender', null,array('class'=>'form-control', 'placeholder'=>'', 'required'=>'email'  )) }} 
		
	  </div> 					
	  <div class="form-group  ">
		<label for="ipt"> {{ Lang::get('core.contact_subject') }} </label>
		
		  {{ Form::text('subject', null,array('class'=>'form-control', 'placeholder'=>Lang::get('core.contact_subject'), 'required'=>'true'   )) }} 
		 
	  </div> 					
	  <div class="form-group  ">
		<label for="ipt"> {{ Lang::get('core.contact_message') }} </label>	
		{{ Form::textarea('message',null,array('class'=>'form-control', 'placeholder'=>Lang::get('core.contact_message2'), 'required'=>'true'   )) }}		 
	  </div>
	  <div class="form-group  ">
		<button type="submit" class="btn btn-primary ">  {{ Lang::get('core.contact_sendf') }}  </button>		 
	  </div>
...
{{ Form::close() }}		
		...
		<div class="col-md-6 ">
		<h3> {{ Lang::get('core.contact_info') }}</h3>
			...
			Email: support@gotongroyong.fund<br/>
			Web: gotongroyong.fund</p>
			<h3> {{ Lang::get('core.contact_sosial') }}</h3>
...
<h3>{{ Lang::get('core.contact_google') }} </h3>
