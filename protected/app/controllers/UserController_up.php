...
class UserController extends BaseController {
	...
	public function postCreate() {
		...
		if ($validator->passes()) {
			...
			$authen->website = Input::get('website');
				...
				if(Auth::check())
				{
					...
					} else if($row->active=='2')
					{
						...
						$_SESSION['ses_id']= $row->id;
                                                if(Session::has('createproject') && !Session::has('postCreateProject')){
                            return Redirect::to('create-project-content');
                        }else if(Session::has('postCreateProject')){
                        	Session::forget('postCreateProject');
                        	return Redirect::to('push-new-project');
                        }
                        else{
                            if(CNF_FRONT =='false') :
	...
	public function postSavepassword()
	{
		...
		{
				$oldPass = Hash::make(Input::get('currentPass'));				
                $newPass = Hash::make(Input::get('newPass'));
                $tampung = Input::get('currentPass');  
       
				$user = User::find(Session::get('uid'));
				$a = $user->password;

				//echo Input::get('currentPass');
				//echo Hash::make(Input::get('oldPass'));
				//exit();

				/*if(Auth::attempt(array('email'=>$user->email,'password'=>Input::get('currentPass')),false,false))
                {                
                    echo "sama  2";
					exit();
				} else {
                ...
                {                
                    ...
                    //return Redirect::to('account')->with('message',SiteHelpers::alert('success','Password has been changed!'));
					return Redirect::to('account')->with('message', "Password has been changed!");
				} else {
                    
                    return Redirect::to('change-password')->with('message', "Old Password Incorrect.");
				}
		}
	...
	{
		$query="select email from tb_newsletter where status = 1";
		$rs= DB::select($query);

		$subject= $_REQUEST["subject_newsletter"];
		$msg= htmlspecialchars($_REQUEST["msg_newsletter"]);

		foreach ($rs as $email_s) 
		{

			$to = $email_s->email;
				if(mail($to, $subject, $msg))
				{
				    return Redirect::to('Newsletter')->with('message',SiteHelpers::alert('success','Your Newsletter has been sent'));
				} 
				else 
				{
				    return Redirect::to('Newsletter')->with('message',SiteHelpers::alert('error','Sending Newsletter Failed'));
				}
		}
	}	
